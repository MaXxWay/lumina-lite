// groups.js — максимально простой, без бесконечных циклов
...
// ПРОСТАЯ ИНИЦИАЛИЗАЦИЯ - ТОЛЬКО ОДИН РАЗ
function initGroups() {
    ...
    if (!supabaseClient || !currentUser || !currentProfile) {
        // Пробуем ещё раз через 1 секунду, но только 3 раза
        if (!window._initGroupsAttempts) {
            window._initGroupsAttempts = 0;
        }
        window._initGroupsAttempts++;
        if (window._initGroupsAttempts < 5) {
            setTimeout(initGroups, 1000);
        }
        return;
    }
    
    console.log('initGroups: инициализация...');
    ...
    try {
        ...
        groupsInitialized = true; // Останавливаем попытки
    }
}
