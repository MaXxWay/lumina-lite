...
async function loadDialogs(searchTerm = '') {
    ...
    try {
        ...
        if (chatsError) {
            ...
            container.innerHTML = '<div class="dialogs-loading">Ошибка загрузки чатов</div>';
            return;
        }
        ...
        if (validChats.length === 0) {
            container.innerHTML = '<div class="dialogs-loading">Нет диалогов. Начните общение через поиск</div>';
